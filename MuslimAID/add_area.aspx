<%@ Page Language="C#" MasterPageFile="~/MuslimAID.Master" AutoEventWireup="true" CodeBehind="add_area.aspx.cs" Inherits="MuslimAID.add_area" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div class="container">
        <div class="PageTitle"><h4>Add Area</h4></div>
        <div class="col-md-6 form-container">
            <div class="form-group row">
                <div class="col-md-5">Branch Code<span style="color:Red;"> *</span></div>
                <div class="col-md-7"><asp:DropDownList CssClass="form-control" ID="cmbCityCode" TabIndex="1" runat="server"></asp:DropDownList></div>
            </div>
            <div class="form-group row">
                <div class="col-md-5">Area<span style="color:Red;"> *</span></div>
                <div class="col-md-7">
                    <asp:TextBox ID="txtArea" CssClass="form-control" 
                        runat="server"></asp:TextBox></div>
            </div>
            <div class="form-group row">
                <div class="col-md-5">Area Code<span style="color:Red;"> *</span></div>
                <div class="col-md-7">
                    <asp:TextBox ID="txtAreaCode" CssClass="form-control" 
                        runat="server" ></asp:TextBox></div>
            </div>
                <asp:Button CssClass="btn btn-primary" ID="btnSubmit" runat="server" 
                Text="Submit" Enabled="true" onclick="btnSubmit_Click"/>&nbsp;
                <asp:Label ID="lblMsg" runat="server"></asp:Label>
        </div>
    </div>
</asp:Content>
